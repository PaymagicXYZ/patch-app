...
const AccountSelector = ({ availableWallets, wallets }: { availableWallets: UserId[]; wallets: string[] }) => {
  ...
  return (
    <div>
      ...
      {selectedAccount && <pre>{wallets[availableWallets.indexOf(selectedAccount)]}</pre>}
    </div>
  );
};
