...
const useDialogStore = create<DialogProps>((set) => ({
  lazyModals: ["SuccessDialog"], // Add dialogs that need to be lazy loaded via the ModalProvider
  openedModals: {
    // successDialog: false,
    sendDialog: {
      opened: false,
...
export const useDialogMeta = (id: DialogId) =>
  useDialogStore((state) => state.openedModals[id].meta);
